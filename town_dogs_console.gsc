//PS3 v1 - TOWN Survival with Dogs
...
init()
{
    if( getdvar( "mapname" ) == "zm_transit" && getdvar ( "g_gametype")  == "zstandard" )
	{
		...
		level.power_up[4] = "fire_sale";
		level.power_up[5] = "random_perk";
		level.power_up[6] = "death_machine";
		level.power_up[7] = "zombie_cash";
		level.power_up[8] = "carpenter";
		level.power_up[9] = "unlimited_ammo";
...
init_custom_map()
{
	...
	perk_system( "script_model", ( 1856, -810.722, -55.875), "zombie_vending_tombstone_on", ( 0, 180, 0 ), "custom", "mus_perks_deadshot_sting", "Deadshot", 2000, "revive_light", "deadshot" );
	...
	perk_system( "script_model", ( 1823.86, 670.574, -55.875 ), "zombie_vending_tombstone_on", ( 0, 45, 0 ), "custom", "mus_perks_doubletap_sting", "Mule Kick", 4000, "doubletap_light", "MULE" );
	...
	perk_system( "script_model", ( 559, -1364, 120.125  ), "zombie_vending_tombstone_on", ( 0, 180, 0 ), "custom", "mus_perks_doubletap_sting", "Ammo Regen", 2500, "marathon_light", "Ammo_Regen" );
}
...
drawshader_and_shadermove(perk, custom, print)
{
    ...
    for(i = 0; i < self.perkarray.size; i++)
	{
        ...
        {    
            self.perk4back = self drawshader( "specialty_marathon_zombies", x, 314, 24, 24, ( 0, 200, 0 ), 100, 0 );
            self.perk4front = self drawshader( "zombies_rank_2", x, 314, 23, 23, ( 1, 1, 1 ), 100, 0 );
        ...
        if(perk == "Burn_Heart")
        {
            self.perk9back = self drawshader( "specialty_marathon_zombies", x, 314, 24, 24, ( 200, 0, 0 ), 100, 0 );
            self.perk9front = self drawshader( "faction_cdc", x, 314, 23, 23, ( 1, 1, 1 ), 100, 0 );
...
custom_powerup_grab(s_powerup, e_player)
{
	...
	{	
		foreach(player in level.players)
	    {
		    ...
		    player thread power_up_hud( "menu_mp_weapons_xm8", 0, "Infinite Ammo!" );
	...
	for(;;)
	{
		...
		while(level.boss_is_alive)
		{
			...
			if(level.inferno.health <= 0)
			{
				...
				specific_powerup_drop(level.power_up[ randomintrange( 0, 10 )], level.inferno.origin );
